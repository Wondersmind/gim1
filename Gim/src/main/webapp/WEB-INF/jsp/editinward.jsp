...
<div class="wrapper">
            ...
            <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Edit Inward</h4></span>
                                        </div>
                                       <div class="actions">
                                       <a class="btn btn-circle btn-icon-only btn-default" id="resrOut" data-toggle="tooltip" title="New">
                                                <i class="icon-plus"></i>
                                            </a>
                                            <a class="btn btn-circle btn-icon-only btn-default" href="javascript:saveinwardaddProduct()" id="saveButtonInIwd" data-toggle="tooltip" title="Update">
                                                <i class="icon-doc"></i>
                                            </a>
                                            ...
                                            <a class="btn btn-circle btn-icon-only btn-default" href="inward.mm" id="backtoback" data-toggle="tooltip" title="Existing">
                                ...
                                <input type="hidden" id="bomExistAll" value='${ExistBomMtrPro}'> 
                                  ...
                                  <input type="hidden" id="authChekForId" value="${extIwdHdrOnly[11]}">
                                     <input type="hidden" id="inwrdPrimId" value="${extIwdHdrOnly[12]}">
                                       <input type="hidden" id="crtDtOfIwrd" value="${extIwdHdrOnly[13]}">
                                         <input type="hidden" id="crtIdOfIwrd" value="${extIwdHdrOnly[14]}">
                                         <input type="hidden" id="ExstGsonInwdDtl" value='${ExstGsonInwdDtl}'>
                                ...
                                <div class="col-md-4">
                                 <select class="selectpicker" data-live-search="true" id="iwdCmyCd">
  													<option value="">Select Company</option>
									                              ...
									                              <c:choose><c:when test="${extIwdHdrOnly[0] eq ExistCom.cm_cmy_cd}">  <option value="${ExistCom.cm_cmy_cd}" selected="selected">${ExistCom.cm_cmy_nm}</option></c:when>
						 ...
						 <option value="">Select Location</option>
						   <c:forEach var="ExistStr" items="${ExistStrDet}">
							   <c:choose><c:when test="${extIwdHdrOnly[1] eq ExistStr.sm_str_cd}"> <option value="${ExistStr.sm_str_cd}" selected="selected">${ExistStr.sm_str_nm}</option></c:when>
                                ...
                                <div class="col-md-10" ng-init="typs='${extIwdHdrOnly[2]}'">
  ...
  <option>OutSide Process</option>
                                ...
                                <div class="col-md-4">
                                 ...
                                 <option value="">Select Vendor</option>
										 <c:forEach var="ExistVn" items="${ExistVnDet}">
										  <c:choose><c:when test="${extIwdHdrOnly[3] eq ExistVn.vcm_vnct_cd}">   <option value="${ExistVn.vcm_vnct_cd}" selected="selected">${ExistVn.vcm_vnct_nm}</option></c:when>
                                ...
                                <div class="col-md-4">
                                <input type="text" class="form-control input-sm" placeholder="Vndr INC No" id="iwdvndrIncNo" value="${extIwdHdrOnly[4]}">
                                </div>
                                </div>
                                <div class="form-group">
                                 <div class="col-md-2">
                                ...
                                <div class="col-md-4">
                                <input type="text" class="form-control input-sm datepicker" placeholder="Vndr INC Date" id="iwdvndrIncDt" value="${extIwdHdrOnly[5]}" disabled>
                                </div>
                                 <div class="col-md-2">
                                <label class="control-label">Department</label>
                                </div>
                                <div class="col-md-4">
                               <select class="selectpicker" data-live-search="true" id="iwdDptcd">
   <option value="">Select DepartMent</option>
									 <c:forEach var="ExistDpt" items="${ExistDptDet}">
										 <c:choose><c:when test="${extIwdHdrOnly[6] eq ExistDpt.dm_dpt_cd}"> <option value="${ExistDpt.dm_dpt_cd}" selected="selected">${ExistDpt.dm_dpt_nm}</option></c:when>
  ...
  <input type="text" class="form-control inptgrp" aria-describedby="basic-addon2" id="srchInInwardModel" autocomplete="off" placeholder="Search by Inward Type {{typs}} Name/Code" onkeydown="if(event.keyCode==13) prfomInwdMdlAdd()"
											onkeypress=" return getAllInwordModelPro(event,this.value)">
											   <c:choose><c:when test="${extIwdHdrOnly[11] eq '1'}"> 
 <span class="input-group-addon ingrdon"><i class="icon-plus"></i></span></c:when>
 <c:otherwise> <span class="input-group-addon ingrdon" id="basic-addon2" onclick="prfomInwdMdlAdd()"><i class="icon-plus"></i></span></c:otherwise> </c:choose>
</div>
                                </div>
                                </div>
                                </div>
                                ...
                                <div class="col-md-6" style="text-align: left;">
                                 <label class="control-label dcment" id="iwdDocNo">${extIwdHdrOnly[7]}</label>
                                ...
                                <div class="col-md-6">
                                <input type="number" class="form-control input-sm" placeholder="Received Wgt" id="rcvdWgtOfInwrd" value="${extIwdHdrOnly[8]}">
                                </div>
                                </div>
                                <div class="form-group">
                                 <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                <input type="number" class="form-control input-sm" placeholder="Received Purity" id="rcvdPrtyOfInwrd" value="${extIwdHdrOnly[9]}">
                                </div>
                                </div>
                                <div class="form-group">
                                 <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                <input type="number" class="form-control input-sm" placeholder="Exp Converted Qty" id="goldPrtyOfInwrd" value="${extIwdHdrOnly[10]}">
                                </div>
                                </div>
                              <div class="form-group">
                                 <div class="col-md-12">
                                   <c:choose><c:when test="${extIwdHdrOnly[11] eq '1'}"> <div class="alert alert-warning" >Inward Is Authentication Mode...</div> </c:when></c:choose>
    ...
    <thead>
      ...
      <th>Prdt/Design Id<input class="form-control input-sm searcher"></th>
       <th>Name<input class="form-control input-sm searcher"></th>
       <th>Qty<input class="form-control input-sm searcher"></th>
        <th>Metal Wgt<input class="form-control input-sm searcher"></th>
        <th>Received Purity<input class="form-control input-sm searcher"></th>
        <th>Document Value<input class="form-control input-sm searcher"></th>
        <th>Program Calculated<input class="form-control input-sm searcher"></th>
        <th>Pure Gold Weight<input class="form-control input-sm searcher"></th>
      </tr>
    </thead>
     <c:choose><c:when test="${extIwdHdrOnly[11] ne '1'}">
    <tbody id="myInwrdBody">
      
    </tbody></c:when></c:choose>
     <tbody id="myTableExist" style="border: aliceblue;">
    <c:forEach var="exstIwd" items="${exstIwdHdrDet}">
    <c:choose><c:when test="${exstIwd[10] eq '0'}">
  <tr  style="background-color: #5fbeaa !important;" value="${exstIwd[9]}">
    </c:when><c:otherwise>
     <tr value="${exstIwd[9]}" style="background-color: #5fbeaa !important">
    </c:otherwise></c:choose>
     
      <td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
       <c:choose><c:when test="${exstIwd[10] eq '0'}">
                                                        <input type="checkbox" class="checkboxes childCheckBoxExist" value="${exstIwd[8]}">
                                                        </c:when>
                                                        <c:otherwise>
                                                           <input type="checkbox" disabled="disabled" class="checkboxes childCheckBoxExist" value="${exstIwd[8]}"></c:otherwise>
                                                        </c:choose><span></span>
                                                    </label></td>
        <td class="pdtIdCmp" id="${exstIwd[0]}=${exstIwd[7]}">${exstIwd[0]}</td>
        <td>${exstIwd[1]}</td>
         <td class="metalQtyTd">${exstIwd[11]}</td>
        <td class="metalWgtTd">${exstIwd[2]}</td>
        <td class="rcvdPrtyTd">${exstIwd[3]}</td>
          <td class="docvalTd">${exstIwd[4]}</td>       
             <td class="pgmCalTd">${exstIwd[5]}</td>    
               <td class="preGldWgtTd">${exstIwd[6]}</td>                                        
      </tr>
      </c:forEach>
    </tbody>
  </table>
  ...
  <script src="js/datatableSMR.js"></script>
	 ...
	 $(function() {
		 if($("#authChekForId").val()=="1"){
				$(".wrapper :input:not(.searcher),.wrapper a:not(#backtoback)").attr("disabled",true);
...
<script src="js/editInward.js"></script>
</body>
</html>