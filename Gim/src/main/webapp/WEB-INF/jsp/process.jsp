...
<div class="wrapper">
  <div class="container-fluid">
            ...
            <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Process Master</h4></span>
                                        </div>
                                        <div class="actions">
                                            <a class="btn btn-circle btn-icon-only btn-default" href="javascript:;" id="resrOut">
                                                <i class="icon-magnifier-remove"></i>
                                            </a>
                                            <a class="btn btn-circle btn-icon-only btn-default" href="addprocess.mm">
                                                <i class="icon-plus"></i>
                                            </a>
                                        ...
                                        <div class="actions">
                                         <c:choose><c:when test="${SaveProcessEditResult eq true}">
    ...
    <tbody>
     <c:forEach var="exstPrcs" items="${exstPrcsDet}">
      <tr>
        <td>${exstPrcs[0]}</td>
        <td>${exstPrcs[1]}</td>
        <td>${exstPrcs[2]}</td>
        ...
        <!--<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                                     ...
                                                     <td class="text-right">
										<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editProcessMstRec(${exstPrcs[5]})"><i class="fa fa-pencil"></i></button>
										...
										<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delProcessMstRec(${exstPrcs[5]},this)"><i class="fa fa-trash-o"></i></button>
									</td>
      </tr>
</c:forEach>
    </tbody>
		...
		var res=eval(tables);
		 if(res !=null && res.length>0){
			 res.forEach((obj,index)=>{
				 if(obj[7])
				 obj[7]=obj[7].split(",").map(s=>dptList.get(s)).join();
				 obj['recid']=index});
			 var tempStorage=res;
				$("#myTable").html('<div id="grid" style="width: 100%; height: 500px;"></div>');
				var columns=[
				              { field: '0', caption: 'Cmy Name',type:'text',sortable: true}, 
					             { field: '1', caption: 'Code',type:'text', sortable: true},
			                    { field: '7',caption:'Dpt Name',type:'text', sortable: true},
			                    { field: '2',caption:'Name',type:'text', sortable: true},
			                    { field: '6',caption:'Process Type',type:'text', sortable: true},
		                        { field: '3', caption: 'Description',type:'text', sortable: true},
		                        { field: '4', caption: 'Status',type:'text', sortable: true},
		                       { field: '8', caption: 'Action',type:'text',
				                            render: function (record, index, column_index) {
				                            	var html='<button type="button" ${LoginDet.em_emp_typ} class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"  onclick="editProcessMstRec(\''+record[5]+'\')"><i class="fa fa-pencil"></i></button>'
				                            		+'&nbsp;<button type="button" ${LoginDet.em_emp_typ} class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delProcessMstRec(\''+record[5]+'\',this)"><i class="fa fa-trash-o"></i></button>';
