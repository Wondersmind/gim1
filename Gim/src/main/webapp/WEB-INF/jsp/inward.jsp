        ...
        <div class="panel panel-default">
          <div class="panel-body">
            ...
            <div class="portlet light bordered">
             <c:choose><c:when test="${saveSucIwdRes eq true}">
                 <span id="finalSaveDesinAlert" style="color:green"><div class="alert alert-success" style="text-align: center;">Saved Successfully</div></span>
                                    ...
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Inward</h4></span>
                                        </div>
                                        <div class="actions">
                                            
                                            <a class="btn btn-circle btn-icon-only btn-default" href="addinward.mm">
                                                <i class="icon-plus"></i>
         ...
         <%--        <table class="table table-bordered" id="myTable">
                    ...
                    <th>Dcmnt No</th>
                     <th>Vndr IncNo</th>
                      ...
                      <tbody>
                      <c:forEach var="exsitInwrd" items="${exsitInwrdDet}">
                    <tr>
                     <td>${exsitInwrd[10]}</td>
                          <td>${exsitInwrd[0]}</td>
                           <td>${exsitInwrd[1]}</td>
                          ...
                          <td>${exsitInwrd[5]}</td>
                            <td>${exsitInwrd[9]}</td>
                         <td class="text-right">
                         <c:choose><c:when test="${exsitInwrd[9] eq '0'}">
										<button type="button" class="btn btn-xs btn-default btn-equal"  data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editinwardRec(${exsitInwrd[8]})"><i class="fa fa-pencil"></i></button>
										
										<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="deleteInwardRec(${exsitInwrd[8]},this)"><i class="fa fa-trash-o"></i></button>
									</c:when>
									<c:otherwise><button type="button" style="width: 50px !important;" class="btn btn-xs btn-default btn-equal"  data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editinwardRec(${exsitInwrd[8]})"><i class="fa fa-pencil"></i></button>
										</c:otherwise>
									</c:choose>
		...
		var res=eval(tables);
		 if(res !=null && res.length>0){
			 res.forEach((obj,index)=>{ 
				 obj[2]=(+obj[2]||0).toFixed(2);
				 obj['recid']=index});
			 var tempStorage=res;
				$("#myTable").html('<div id="grid" style="width: 100%; height: 500px;"></div>');
				var columns=[
				             { field: '10', caption: 'CmyName',type:'text', sortable: true},
		                        { field: '0', caption: 'DocNo',type:'text',sortable: true},
		                        { field: '13', caption: 'Doc.DT',type:'text',sortable: true},
		                        { field: '11', caption: 'Type',type:'text', sortable: true},
		                        { field: '1',caption:'Invc No',type:'text', sortable: true },
		                        { field: '2', caption: 'Invc Dt',type:'float', sortable: true},
		                        { field: '3', caption: 'Rcvd.wgt',type:'float', sortable: true},
		                        { field: '12', caption: 'Qty',type:'float', sortable: true},
		                        { field: '4', caption: 'Rcvd.Prty',type:'float', sortable: true},
		                        { field: '6', caption: 'Doc.Val',type:'float', sortable: true},
		                        { field: '7', caption: 'Pgm.Cal.Val',type:'float', sortable: true},
		                        { field: '5', caption: 'Pre.Gld.Wgt',type:'float', sortable: true},
		                        { field: '9', caption: 'Auth.Sts',type:'text',size:"50", sortable: true},
		                        { field: '10', caption: 'Action',type:'text',
				                            render: function (record, index, column_index) {
				                            	var html='';
				                            	if(record[9]=='1'){
				                            	html='<button type="button" style="width: 48px !important;" class="btn btn-xs btn-default btn-equal"  data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editinwardRec(\''+record[8]+'\')"><i class="fa fa-pencil"></i></button>';	
				                            	}
				                            	else{
				                            		html='<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editinwardRec(\''+record[8]+'\')"><i class="fa fa-pencil"></i></button>';
													}
			           ...
			           if(trgt.force)
			        	{
			        	   ...
			        	   delInwardRecordRec($.trim(tds[1].innerText));
			        	}
			        } ,
			        reorderColumns: true,
			        multiSelect:false,
...
function delInwardRecordRec(doc){
	var qry="exec deleterecordsfrominternaltransfer @cmycd='"+cmyCode+"',@primid='0',@docno='"+doc+"',@trnstyp='Inward' "
