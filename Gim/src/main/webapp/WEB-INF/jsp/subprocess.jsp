        ...
        <div class="panel panel-default">
          <div class="panel-body">
            ...
            <div class="portlet light bordered">
             <c:choose><c:when test="${savedResOfSupPrcsShow eq true}">
                 <span id="finalSaveDesinAlert" style="color:green"><div class="alert alert-success" style="text-align:center;">Saved Successfully</div></span>
                                    ...
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Sub Process</h4></span>
                                        </div>
                                        <div class="actions">
                                            <a class="btn btn-circle btn-icon-only btn-default" href="javascript:;" id="resrOut">
                                                <i class="icon-magnifier-remove"></i>
                                            </a>
                                            <a class="btn btn-circle btn-icon-only btn-default" href="addsubprocess.mm">
                                                <i class="icon-plus"></i>
                                            </a>
              ...
              <div class="table-responsive"  id="myTable">
               ...
               <%--  <table class="table table-bordered" id="myTable">
                    ...
                    <th>Document No</th>
                      ...
                      <tbody>
                      <c:forEach var="ExtSubPrsDet" items="${ExstSubPrcsDet}">
                    <tr>
                          <td>${ExtSubPrsDet[0]}</td>
                           <td>${ExtSubPrsDet[1]}</td>
                          <td>${ExtSubPrsDet[2]}</td>
                          <td>${ExtSubPrsDet[3]}</td>
                          <td>${ExtSubPrsDet[4]}</td>
                          <td>${ExtSubPrsDet[5]}</td>
                          <td>${ExtSubPrsDet[6]}</td>
                          <td>${ExtSubPrsDet[7]}</td>
                         
                         <td class="text-right">
										<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editsubprcs(${ExtSubPrsDet[8]},'${ExtSubPrsDet[0]}','${ExtSubPrsDet[2]}')"><i class="fa fa-pencil"></i></button>
										...
										<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="deletesubprcshdr(${ExtSubPrsDet[8]},'${ExtSubPrsDet[0]}',this)"><i class="fa fa-trash-o"></i></button>
									</td>
                        </tr>
               </c:forEach>
                  </tbody>
		...
		var res=eval(tables);
		 if(res !=null && res.length>0){
			 ...
			 var tempStorage=res;
				...
				var columns=[
				             { field: '10', caption: 'Cmy Name',type:'text', sortable: true},
		                        { field: '0', caption: 'Dept',type:'text',sortable: true},
		                        { field: '1',caption:'Doc No',type:'text', sortable: true},
		                        { field: '12',caption:'Doc Dt',type:'text', sortable: true},
		                        { field: '2', caption: 'Process',type:'text', sortable: true},
		                        { field: '11', caption: 'Wrk Typ',type:'text', sortable: true},
		                        { field: '3', caption: 'Worker',type:'text', sortable: true},
		                        { field: '4',caption: 'Isd.Wgt',type:'float', sortable: true},
		                        { field: '5', caption: 'Rcvd.Wgt',type:'float', sortable: true},
		                        { field: '6', caption: 'Bal.Wgt',type:'float', sortable: true},
		                        { field: '7', caption: 'Isd.Auth',type:'text', sortable: true},
		                        { field: '8', caption: 'Rcvd.Auth',type:'text', sortable: true},
		                        { field: '14',caption:' Rcvd.Purity',type:'text', sortable: true},
		                        { field: '13',caption:'Mtl.Purity',type:'text', sortable: true},
		                        { field: '9', caption: 'Action',type:'text',
				                            render: function (record, index, column_index) {
				                            	var html='';
				                            	if(record[7]=='1'){
				                            	html='<button type="button" style="width: 48px !important;" class="btn btn-xs btn-default btn-equal"  data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editsubprcs(\''+record[9]+'\',\''+record[1]+'\',\''+record[2]+'\')"><i class="fa fa-pencil"></i></button>';	
				                            	}
				                            	else{
				                            		html='<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editsubprcs(\''+record[9]+'\',\''+record[1]+'\',\''+record[2]+'\')"><i class="fa fa-pencil"></i></button>'
				                            		+'<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="deletesubprcshdr(\''+record[9]+'\',\''+record[1]+'\',this)"><i class="fa fa-trash-o"></i></button>';
													}
				                            	 	return html;
				                            } 
				                        }
...
function editsubprcs(primId,docNo,trnTyp){
	window.location.href="editsubprocess.mm?sph_id="+primId+"&sph_doc_no="+docNo+"&sph_trn_typ="+trnTyp+"";
}

function deletesubprcshdr(primId,docNo,ths){
	$(ths).closest("tr").css("background-color","orangered");
	AjaxController.deletesubprcshdrDtlAjax(docNo,returnResOfDelAlyHdr);
