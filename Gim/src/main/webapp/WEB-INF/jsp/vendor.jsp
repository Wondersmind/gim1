            ...
            <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Vendor/Customer Master</h4></span>
                                        </div>
                                        <div class="actions">
                                            <a class="btn btn-circle btn-icon-only btn-default" href="javascript:;" id="resrOut">
                                                <i class="icon-magnifier-remove"></i>
                                            </a>
                                            <a class="btn btn-circle btn-icon-only btn-default" href="addvendor.mm">
                                                <i class="icon-plus"></i>
                                            </a>
                                        ...
                                        <div class="actions">
                                          <c:choose><c:when test="${SaveVendorEditResult eq true}">
...
<th>Status</th>
    ...
    <tbody>
    <c:forEach var="exstVendor" items="${exstVendorDetail}">
      <tr>
        <td>${exstVendor[0]}</td>
        <td>${exstVendor[1]}</td>
        <td>${exstVendor[2]}</td>
        ...
        <td>${exstVendor[11]}</td>
        <td>${exstVendor[12]}</td>
        <td>${exstVendor[13]}</td>        <td class="text-right">
										<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editVendorMstRec(${exstVendor[12]})"><i class="fa fa-pencil"></i></button>
										...
										<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delVendorMstRec(${exstVendor[12]},this)"><i class="fa fa-trash-o"></i></button>
									</td>
      </tr>
</c:forEach>
    </tbody>
...
<script>
var dptList=eval('${ExstDptNmVsCd}'||'[]');
dptList=dptList.reduce((p,n)=>(p.set(n[0],n[1]),p),new Map());
		...
		var res=eval(tables);
		 if(res !=null && res.length>0){
			 res.forEach((obj,index)=> {
				 if(obj[14])
				 obj[14]=obj[14].split(",").map(s=>dptList.get(s)).join();
				 obj['recid']=index});
			 var tempStorage=res;
				$("#myTable").html('<div id="grid" style="width: 100%; height: 500px;"></div>');
				var columns=[
				              { field: '0', caption: 'Cmy Nm',type:'text',sortable: true}, 
					             { field: '1', caption: 'Vendor Cd',type:'text', sortable: true},
			                    { field: '2',caption:'Vendor Nm',type:'text', sortable: true},
			                    { field: '14', caption: 'Dept',type:'text', sortable: true},
		                        { field: '3', caption: 'Address',type:'text', sortable: true},
		                        { field: '5', caption: 'City',type:'text', sortable: true},
		                        { field: '6', caption: 'State',type:'text', sortable: true},
		                        {field: '9', caption: 'Alw Qty',type:'text', sortable: true},
		                        { field: '10', caption: 'Mob.No',type:'text', sortable: true},
		                        { field: '11', caption: 'Email',type:'text', sortable: true},
		                        { field: '13', caption: 'Status',type:'text', sortable: true},
		                       { field: '8', caption: 'Action',type:'text',
				                            render: function (record, index, column_index) {
				                            	var html='<button type="button" ${LoginDet.em_emp_typ} class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"  onclick="editVendorMstRec(\''+record[12]+'\')"><i class="fa fa-pencil"></i></button>'
				                            		+'&nbsp;<button type="button" ${LoginDet.em_emp_typ} class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delVendorMstRec(\''+record[12]+'\',this)"><i class="fa fa-trash-o"></i></button>';
