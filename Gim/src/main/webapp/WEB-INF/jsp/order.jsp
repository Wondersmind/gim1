...
<div class="wrapper">
  <div class="container-fluid">
            ...
            <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Order</h4></span>
                                        </div>
                                        <div class="actions">
                                          <a class="btn btn-circle btn-icon-only btn-default" href="importorder.mm" data-toggle="tooltip" title="Import">
                                                <i class="icon-cloud-download"></i>
                                            </a>
                                        
                                            <a class="btn btn-circle btn-icon-only btn-default" href="addorder.mm">
                                                <i class="icon-plus"></i>
                                            </a>
    ...
    <tbody>
    <c:forEach var="ExsitOrd" items="${exstOrdDet}">
    <tr>
    <td>${ExsitOrd[9]}</td>
      <td>${ExsitOrd[0]}</td>
       <td>${ExsitOrd[1]}</td>
       ...
       <td>${ExsitOrd[6]}</td>
          <td>${ExsitOrd[8]}</td>
       <td class="text-right">		<!--<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Copy row"><i class="fa fa-copy"></i></button>-->
										<c:choose><c:when test="${ExsitOrd[8] eq '0'}">
										<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"  onclick="editOrderHdrRec(${ExsitOrd[7]})"><i class="fa fa-pencil"></i></button>
									<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delOrderHdrRec(${ExsitOrd[7]},this)"><i class="fa fa-trash-o"></i></button>
									</c:when><c:otherwise>
									<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" style="width:48px"  onclick="editOrderHdrRec(${ExsitOrd[7]})"><i class="fa fa-pencil"></i></button>
		...
		var res=eval(tables);
		 if(res !=null && res.length>0){
			 ...
			 var tempStorage=res;
				...
				var columns=[
				             { field: '9', caption: 'CmyName',type:'text', sortable: true},
		                        { field: '0', caption: 'OrderId',type:'text',sortable: true},
		                        { field: '10', caption: 'Ord.DT',type:'text',sortable: true},
		                        { field: '1',caption:'CstmrName',type:'text', sortable: true},
		                        { field: '13',caption:'Jo No',type:'text', sortable: true},
		                        { field: '2', caption: 'BatchNo',type:'text', sortable: true},
		                        { field: '3', caption: 'No.Of.Pdts',type:'float', sortable: true},
		                        { field: '4',caption: 'TOt.Qty',type:'float', sortable: true},
		                        { field: '14',caption: 'Carat',type:'text', sortable: true},
		                        { field: '5', caption: 'Sum.Of.Std.wgt',type:'float', sortable: true,
		                        	render: function (record, index, column_index) {
		                        		var num=Number(record[5])||0;
		                        		return 	num.toFixed(2);
		                        	}
		                        },
		                        { field: '8', caption: 'Auth.Sts',type:'text', sortable: true},
		                        { field: '8', caption: 'Action',type:'text',
				                            render: function (record, index, column_index) {
				                            	var html='';
				                            	if(record[8]=='1'){
				                            	html='<button type="button" style="width: 48px !important;" class="btn btn-xs btn-default btn-equal"  data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editOrderHdrRec(\''+record[7]+'\')"><i class="fa fa-pencil"></i></button>';	
				                            	}
				                            	else{
				                            		html='<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editOrderHdrRec(\''+record[7]+'\')"><i class="fa fa-pencil"></i></button>'
				                            		+'<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delOrderHdrRec(\''+record[7]+'\',this)"><i class="fa fa-trash-o"></i></button>';
													}
				                            	return html;
				                            } 
				                        }
