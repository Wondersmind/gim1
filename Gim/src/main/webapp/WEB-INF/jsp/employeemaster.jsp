...
<div class="wrapper">
  <div class="container-fluid">
            ...
            <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            ...
                                            <span class="caption-subject font-dark bold uppercase text-muted page-title-alt"> <h4 class="page-title">Employee Master</h4></span>
                                        </div>
                                        <div class="actions">
                                            <a class="btn btn-circle btn-icon-only btn-default" href="javascript:;" id="resrOut">
                                                <i class="icon-magnifier-remove"></i>
                                            </a>
                                            <a class="btn btn-circle btn-icon-only btn-default" href="addemployee.mm">
                                         ...
                                         <div class="actions">
                                         <c:choose><c:when test="${SaveEmpEditResult eq true}">
    ...
    <tbody>
    <c:forEach var="ExistEmp" items="${ExistEmpDetail}">
      <tr>
        <td>${ExistEmp[0]}</td>
        <td>${ExistEmp[1]}</td>
        <td>${ExistEmp[2]}</td>
        ...
        <td>${ExistEmp[9]}</td>
                                 <td class="text-right">
										<button type="button" class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row" onclick="editEmpMstRec(${ExistEmp[10]})"><i class="fa fa-pencil"></i></button>
										...
										<button type="button" class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delEmpMstRec(${ExistEmp[10]},this)"><i class="fa fa-trash-o"></i></button>
									</td>
      </tr>
</c:forEach>
    </tbody>
		...
		var res=eval(tables);
		 if(res !=null && res.length>0){
			 res.forEach((obj,index)=> {
				 obj['recid']=index;
				 if(obj[2])
				 obj[2]=obj[2].split(",").map(s=>dptList.get(s)).join();
				 });
			 var tempStorage=res;
				$("#myTable").html('<div id="grid" style="width: 100%; height: 500px;"></div>');
				var columns=[
				              { field: '0', caption: 'Cmy Name',type:'text',sortable: true}, 
					             { field: '1', caption: 'Dcmnt No',type:'text', sortable: true},
			                    { field: '2',caption:'Dept Name',type:'text', sortable: true},
		                        { field: '3', caption: 'Name',type:'text', sortable: true},
		                        { field: '4', caption: 'Number',type:'text', sortable: true},
		                        { field: '5', caption: 'Email',type:'text',sortable: true}, 
					             { field: '6', caption: 'Login',type:'text', sortable: true},
			                    { field: '8', caption: 'Type',type:'text', sortable: true},
		                        { field: '9', caption: 'AllowQty',type:'text', sortable: true},
		                        { field: '8', caption: 'Action',type:'text',
				                            render: function (record, index, column_index) {
				                            	var html='<button type="button" ${LoginDet.em_emp_typ} class="btn btn-xs btn-default btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"  onclick="editEmpMstRec(\''+record[10]+'\')"><i class="fa fa-pencil"></i></button>'
				                            		+'&nbsp;<button type="button" ${LoginDet.em_emp_typ} class="btn btn-xs btn-danger btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="Delete row" onclick="delEmpMstRec(\''+record[10]+'\',this)"><i class="fa fa-trash-o"></i></button>';
