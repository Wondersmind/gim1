...
(function(h) {
    ...
    m.defaults = {
        ...
        fnStateLoadCallback: function(a) {
            try {
                return eval((-1 === a.iStateDuration ?
        ...
        oLanguage: {
            ...
            sSearch: "",
            sSearchPlaceholder: "Search All Field Here...",
            sUrl: "",
...
$.fn.avoidDubOption=function(){
	var length=$(this).length;
	for(var i=0;i<length;i++){
		var seter=new Set();
		$($(this)[i]).find("option").toArray().forEach(s=>{
			if($(s).html())
		seter.add($(s)[0].outerHTML);
	});
		$($(this)[i]).html([...seter]);
	}
}
...
function tablerefresh(tableid){
	...
	if(tableid=="tableoffinsh")option.columnDefs=[{ type: 'date-dd-mmm-yyyy', targets: 1}];
	if(tableid=='tableofdc')option.columnDefs=[{ type: 'date-dd-mmm-yyyy', targets: 2}];
...
table.columns().eq( 0 ).each( function ( colIdx ) {
	    $( 'input:not(:checkbox)', table.column( colIdx ).header() ).on( 'keyup change', function () {
	        ...
	        if(location.href.search(/\addfinish/)>-1){
	        	try{
	        	totvaluecalculator();
	        	}
	        	catch(e){
	        		console.log("Hi--totvaluecalculator is Not Found ")
	        	}
	        }
	    } );});
};
...
var customDateDDMMMYYYYToOrd = function (date) {
	...
	var today=$("<input/>").datepicker("setDate",new Date());
	if(!date)date=today.val();
...
function showProductDetail(tpd){
	var pdtCd=(this==window?tpd:this).innerText;
	var qry="select dm_dgn_carat,dm_dgn_ctgy,(select top 1 dm_dpt_nm from dpt_mst where dm_dpt_cd=dm_dgn_dpt and dm_sts=1) as dpt,dm_dgn_img,dm_dgn_max_wt,dm_dgn_min_wt,dm_dgn_no,dm_dgn_std_wt,dm_dgn_sz,dm_dgn_typ,dm_id from dgn_mst where dm_pdt_cd='"+pdtCd+"' and dm_dgn_sts=1"
	AjaxController.getProductDetail(qry,(data)=>{
		if(data&&data.length){
			var pdtcrt=data[0][0]||'',ctgy=data[0][1]||'',img=data[0][3]||'',dpt=data[0][2]||'',sz=data[0][8]||'',
			minmax=(data[0][5]||'')+"-"+(data[0][4]||''),dgnno=data[0][6]||'',stdwgt=data[0][7]||'',typ=data[0][9]||'',
			primId=data[0][10]||0;
			$(".PofCD").html(pdtCd);$(".PofDgnNO").html(dgnno);$(".PofDpt").html(dpt);$(".PofSz").html(sz);$(".PofCart").html(pdtcrt);
			$(".PofCtgy").html(ctgy);$(".PofTyp").html(typ);$(".PofMinMax").html(minmax);$(".PofStdWgt").html(stdwgt);
			$("#PofImgViewer").html('<img src="showImageUrl.mm?path='+img+'" style="height: 160px;width: 280px;" alt="teste" class="img-thumbnail">  ');
		AjaxController.getBomDetailByPdtCd(primId,'BOM',(data)=>{
			var html='';
			if(data&&data.length){
			data.forEach(tr=>{
				html+='<tr><td>'+tr[0]+'</td><td>'+tr[1]+'</td><td>'+tr[2]+'</td><td>'+tr[3]+'</td><td>'+tr[4]+'</td><td>'+tr[5]+'</td></tr>';
			})	
			}
			$("#productView").attr("PrdtPrimId",primId);
			$("#bomTable").html(html);$("#bomCt a").click();
...
$("#moldCt,#stnCt").click(function(){
	var primId=$("#productView").attr("PrdtPrimId")||0;
	if(this.id=='moldCt'){
		AjaxController.getBomDetailByPdtCd(primId,'MOLD',(data)=>{
			var html='';
			if(data&&data.length){
			data.forEach(tr=>{
				tr[1]=tr[1]||'';
				html+='<tr><td>'+tr[0]+'</td><td>'+tr[1]+'</td><td>'+tr[2]+'</td></tr>';
			})	
			}
			$("#moldTable").html(html);
		});
				...
				AjaxController.getBomDetailByPdtCd(primId,'STONE',(data)=>{
					var html='';
					if(data&&data.length){
					data.forEach(tr=>{
						html+='<tr><td>'+tr[0]+'</td><td>'+tr[1]+'</td><td>'+tr[2]+'</td><td>'+tr[3]+'</td><td>'+tr[4]+'</td><td>'+tr[5]+'</td></tr>';
...
$("tbody").delegate(".covertedPrtys,.expectedprtyofaly,.rtnPrtyies","blur",function(){
	if(this.value)this.value=(+this.value||0).toFixed(2);
});
$("tbody").delegate(".issuedMtelWght,.rcvdWgtOdMtl","blur",function(){
	if(this.value)this.value=(+this.value||0).toFixed(3);
});