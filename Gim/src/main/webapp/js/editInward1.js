	...
	if($(".childCheckBox").length>0||$(".childCheckBoxExist:not(:disabled)").length>0){
		...
		if($("#isauthVal").val()=="1"){
			...
			$(".childCheckBoxExist:checked:not(:disabled)").each(function (){
				...
				pgmCl=$(this).closest("tr")["0"].cells["6"].innerHTML,pureGldWt=$(this).closest("tr")["0"].cells["6"].innerHTML,pdtName=$(this).closest("tr")["0"].cells["2"].innerHTML,typInw=$(this).closest("tr").attr("value");
				InwrdHdrDtlExist.push({ihd_doc_no:docNo,ihd_cmy_cd:cmycd,ihd_str_cd:strcd,ihd_iwd_typ:typInw,ihd_vndr_cd:vndrcd,ihd_vndr_inc_no:incno,ihd_vndr_inc_dt:incdt,ihd_dpt_cd:dptcd,ihd_pdt_cd:pdtCd,
					ihd_mtl_wgt:mtWgt,ihd_rcvd_prty:rvdPrty,ihd_doc_val:docVal,ihd_pgm_cal:pgmCl,ihd_pure_gld_wgt:pureGldWt,ihd_iwd_sts:true,ihd_pdt_nm:pdtName,ihd_iss_auth:false});	
			});
			var stkJson=[];
		...
		$.grep(InwrdHdrDtl,function(i){
			...
			if(stkJson.length>0){
				var newStk=stkJson.find(function(stk){
					if(stk.stm_itm_cd==i.ihd_pdt_cd&&stk.stm_stk_itm_typ==i.ihd_iwd_typ&&stk.stm_rcvd_stk_prty==i.ihd_rcvd_prty){
					return true;
					}
				});
				if(newStk!=null&&newStk!=undefined){
					...
					iwdStkPro.push(newStk)	
						...
						iwdStkPro.push({stm_str_cd:i.ihd_str_cd,stm_cmy_cd:i.ihd_cmy_cd,stm_itm_cd:i.ihd_pdt_cd,stm_stk_trn_typ:'Inward',stm_stk_itm_typ:i.ihd_iwd_typ,stm_rcvd_stk_wgt:i.ihd_mtl_wgt,stm_rcvd_stk_prty:i.ihd_rcvd_prty,
							stm_stk_pure_gld_wgt:i.ihd_pure_gld_wgt,stm_stk_pure_doc_val:i.ihd_doc_val,stm_stk_pure_pgm_cal:i.ihd_pgm_cal,stm_dpt_cd:i.ihd_dpt_cd});
						}
			...
			iwdItmMv.push({im_cmy_cd:i.ihd_cmy_cd,im_str_cd:i.ihd_str_cd,im_trn_typ:'Inward',im_trn_nmb:i.ihd_doc_no,im_itm_cd:i.ihd_pdt_cd,im_itm_wgt:i.ihd_mtl_wgt
				,im_itm_prty:i.ihd_rcvd_prty,im_itm_pure_gld_wgt:i.ihd_pure_gld_wgt,im_itm_typ:i.ihd_iwd_typ,im_dpt_cd:i.ihd_dpt_cd}); 
		});
		$.grep(InwrdHdrDtlExist,function(i){
			i.ihd_iss_auth=true;
			if(stkJson.length>0){
				var newStk=stkJson.find(function(stk){
					if(stk.stm_itm_cd==i.ihd_pdt_cd&&stk.stm_stk_itm_typ==i.ihd_iwd_typ&&stk.stm_rcvd_stk_prty==i.ihd_rcvd_prty){
				...
				if(newStk!=null&&newStk!=undefined){
					stk.stm_rcvd_stk_wgt=Number(stk.stm_rcvd_stk_wgt)+Number(i.ihd_mtl_wgt);
					stk.stm_stk_pure_gld_wgt=Number(stk.stm_stk_pure_gld_wgt)+Number(i.ihd_pure_gld_wgt);
					stk.stm_stk_pure_doc_val=Number(stk.stm_stk_pure_doc_val)+Number(i.ihd_doc_val);
					stk.stm_stk_pure_pgm_cal=Number(stk.stm_stk_pure_pgm_cal)+Number(i.ihd_pgm_cal);
					iwdStkPro.push(stk);	
				}
					else{
						iwdStkPro.push({stm_str_cd:i.ihd_str_cd,stm_cmy_cd:i.ihd_cmy_cd,stm_itm_cd:i.ihd_pdt_cd,stm_stk_trn_typ:'Inward',stm_stk_itm_typ:i.ihd_iwd_typ,stm_rcvd_stk_wgt:i.ihd_mtl_wgt,stm_rcvd_stk_prty:i.ihd_rcvd_prty,
		...
		AjaxController.saveInwardAddProductInAjax(JSON.stringify(InwrdHdrDtl),JSON.stringify(iwdStkPro),JSON.stringify(iwdItmMv),JSON.stringify(iwdHdrRec),true,sucResOfIwdDtl);	
		}
	else{
