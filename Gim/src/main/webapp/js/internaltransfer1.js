...
$(function (){
	$("#frmDptOfIntTrns,#transTypeOfIntTrns,#cmyCdOfIntlTrf,#filterOfPdtDeptInJobCd").on("change",function(){
		...
		var qrychce='s.stm_stk_trn_typ like \'%Inward%\' or s.stm_stk_trn_typ like \'%InternalProcess%\' or s.stm_stk_trn_typ like \'%JobCardMaking%\'';
		if(frmDpt!=""&&trnTyp!=""&&cmyCd!="" && trnTyp!='JobCard'){
			...
			$(".pdtDptInjbCdHde:not(:visible)").show();
				...
				var qry="select 'JobCard' as trftyp,stm_itm_cd,stm_dpt_cd,stm_rcvd_stk_wgt,(select top 1 min(cast(johd_trgt_dt as date)) from job_ord_hdr_dtl where johd_doc_no=stm_itm_cd) as trgtdt," +
						...
						",stm_id from stk_mst where stm_stk_trn_typ='JobCardRecieved' and stm_dpt_cd=\'"+$("#frmDptOfIntTrns").val()+"\' and stm_cmy_cd=\'"+cmyCd+"\' and stm_rcvd_stk_wgt>'0.0'";
	...
	if(res!=null){
		if(trnTypOfControl=='Tree No'){
			...
			for(var i=0;i<res.length;i++){
				...
				if(balwgt>0){
				result+='<tr><td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline" >'
			           +'<input type="checkbox" class="checkboxes childCheckBox" value="'+res[i][6]+'" onchange="childCkeckBoxChecked()"> <span></span> </label></td><td>'+res[i][0]+'</td><td>'+res[i][1]+'</td><td>'+balwgt+'</td><td><input type="number" class="form-control smplIssWgtOfCst" style=""   max="'+balwgt+'" value="'+balwgt+'"></td><td class="itemPurityNew">'+res[i][5]+'</td>'
			           +'<td>'+res[i][3]+'</td></tr>'	;
				drpDown[res[i][1]]=res[i][1];
				}
			}
...
function saveInternalTrfHdr(){
	...
	if(res==null){
			...
			var itlTrfHdr={ith_doc_no:docno,ith_frm_dpt_cd:frmDpt,ith_to_dpt_cd:toDpt,ith_trf_typ:trType,ith_tot_rcvd_wgt:totrcvdWgt,ith_tot_iss_wgt:totIsdWgt,ith_iss_auth:false,ith_trf_sts:true}; 
			...
			if(trType=='Tree No'){
				$(".childCheckBox:checked").each(function(){
					...
					intlTrfHdrDtl.push({ithd_tree_no:tresNo,ithd_itm_rcvd_wgt:rcvdwgt,ithd_itm_typ:'',ithd_itm_trn_typ:trnTyp,ithd_doc_no:docno,ithd_cmy_cd:cmyCd,ithd_str_cd:strCd,ithd_frm_dpt_cd:frmDpt,ithd_to_dpt_cd:toDpt,
				...
				if($("#CstingstkForEdit").val()!=""&&$("#isdauthVal").val()=="1"){
					var stkDtlList=eval($("#CstingstkForEdit").val());
				var exstJobCrdIssdStkStr=$("#IsuedJobCardMakingForEdit").val();
					var exstJbCdIsdStk=[];
				if(exstJobCrdIssdStkStr!=""){
					exstJbCdIsdStk=eval(exstJobCrdIssdStkStr);
				}
				$(".childCheckBox:checked").each(function(){
				...
				,treeNo=$(trOne[7]).html();
				intlTrfHdrDtl.push({ithd_itm_trn_typ:'JobCard',ithd_itm_rcvd_wgt:bmwgt,ithd_itm_iss_wgt:isdWgt,ithd_ord_no:OrdNo,ithd_tree_no:treeNo,ithd_doc_no:docno,ithd_cmy_cd:cmyCd,ithd_str_cd:strCd,ithd_frm_dpt_cd:frmDpt,ithd_to_dpt_cd:toDpt,
				...
				if($("#isdauthVal").val()=="1"){
					if(!$("#filterOfPdtDeptInJobCd").val()){
						...
						var exstJobCardList=eval(JobCardRecieved);
				...
				if($("#ExsitStkDtlStr").val()!=""&&$("#isdauthVal").val()=="1"){
					var stkDtlList=eval($("#ExsitStkDtlStr").val());
				...
				if(exstIseIntlTrfStr!=""){
					exstIseIntlTrf=eval(exstIseIntlTrfStr);
					}
				if(exstInwardsStkStr!=""){
					exstInwardsStk=eval(exstInwardsStkStr);
				}
				$(".childCheckBox:checked").each(function(){
